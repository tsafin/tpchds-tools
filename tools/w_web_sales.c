...
void (*g_w_web_sales_callback)(const struct W_WEB_SALES_TBL *row, void *ctx) = NULL;
void *g_w_web_sales_callback_ctx = NULL;
   ...
   tdef *pT = getSimpleTdefsByNumber(WEB_SALES);
	...
	nullSet(&pT->kNullBitMap, WS_NULLS);
      ...
      */
      ...
      {
#ifdef EMBEDDED_DSDGEN
         mk_w_web_returns(&w_web_returns, g_w_web_sales_callback ? 0 : 1);
         if (!g_w_web_sales_callback && bPrint)
#else
      ...
      */
#ifdef EMBEDDED_DSDGEN
      if (g_w_web_sales_callback)
         g_w_web_sales_callback(r, g_w_web_sales_callback_ctx);
      else
#endif /* EMBEDDED_DSDGEN */
      if (bPrint)
		  pr_w_web_sales(NULL);
