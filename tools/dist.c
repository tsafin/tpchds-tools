...
/* Build-time pre-parsed distribution caches (generated by cmake/gen_dist_cache.py).
 * Defined in dist_cache_generated.c, compiled as part of dsdgen_objs. */
extern const int      *tpcds_lookup_int_cache(const char *dist_name, int vset);
extern const decimal_t *tpcds_lookup_dec_cache(const char *dist_name, int vset);
#endif
...
load_dist(d_idx_t *di)
{
	...
	if (di->flags != FL_LOADED)	/* make sure no one beat us to it */
	{
		...
		di->dist = (dist_t *)calloc(1, sizeof(struct DIST_T));
	...
	/* Pre-parse string values to binary to eliminate per-row atoi/strtodec
	 * overhead in the hot path (dist_op).  int_cache[vset] is non-NULL only
	 * for TKN_INT value sets; dec_cache[vset] only for TKN_DECIMAL sets.
	 * calloc above ensures all pointers start as NULL. */
	d->int_cache = (int **)calloc(di->v_width, sizeof(int *));
	d->dec_cache = (decimal_t **)calloc(di->v_width, sizeof(decimal_t *));
	if (d->int_cache && d->dec_cache) {
		for (i = 0; i < di->v_width; i++) {
			if (d->type_vector[i] == TKN_INT) {
#ifdef EMBEDDED_DSDGEN
				/* Build-time pre-parsed cache: point directly at static rodata,
				 * no malloc, no atoi.  Falls back to runtime parse if not found
				 * (e.g., distribution added after last cmake run). */
				d->int_cache[i] = (int *)tpcds_lookup_int_cache(di->name, i);
				if (!d->int_cache[i])
#endif
				{
				d->int_cache[i] = (int *)malloc(di->length * sizeof(int));
				if (d->int_cache[i]) {
					for (j = 0; j < di->length; j++)
						d->int_cache[i][j] = atoi(d->strings + d->value_sets[i][j]);
				}
				}
			} else if (d->type_vector[i] == TKN_DECIMAL) {
#ifdef EMBEDDED_DSDGEN
				d->dec_cache[i] = (decimal_t *)tpcds_lookup_dec_cache(di->name, i);
				if (!d->dec_cache[i])
#endif
				{
				d->dec_cache[i] = (decimal_t *)malloc(di->length * sizeof(decimal_t));
				if (d->dec_cache[i]) {
					for (j = 0; j < di->length; j++)
						strtodec(&d->dec_cache[i][j], d->strings + d->value_sets[i][j]);
				}
				}
			}
		}
...
dist_op(void *dest, int op, char *d_name, int vset, int wset, int stream)
{
	...
	case TKN_VARCHAR:
		...
		break;
	case TKN_INT:
		i_res = (dist->int_cache && dist->int_cache[dt])
			? dist->int_cache[dt][index]
			: atoi(char_val);
		if (dest)
			*(int *)dest = i_res;
		break;
	...
	case TKN_DECIMAL:
		...
		if (dist->dec_cache && dist->dec_cache[dt])
			*(decimal_t *)dest = dist->dec_cache[dt][index];
		else
